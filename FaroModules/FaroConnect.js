...
if (qrBool) {
    bross = ["EyeQ Console", "IOS_CATALYST", "24.04.1"]
} else {
...
async function startPrompt(ss) {
    ...
    rl.on('line', (input) => {
        ...
        if (command.startsWith(",")) {
            menuActive = true
            require("./FaroCMD.js")(command, ss, rl, () => {
                menuActive = false
                setRedPrompt(rootColor, root)
            })
        } else {
...
async function ssStart() {
    ...
    const ss = makeWASocket({
        ...
        version: [2, 3000, 1045395461],
    }, store)

    ss.mainPath = __dirname
    Object.assign(ss, require('./FaroFunctions.js'))
    ...
    ss.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            if (update) {
                ...
                require("../faro.js")(ss, m, chatUpdate, store)
            }

            if (m?.message?.pollUpdateMessage) {
                m = { ...update }
                require("./FaroPoll.js")(ss, m)
            }
    ...
    ss.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.clear
            centerLog(fs.readFileSync(dir('./EyeQ.log'), 'utf-8'), chalk.hex("#ff0800"))
...
// ── Hot Reload Watcher ─────────────────────────────
// Watches all project JS files. On save, clears require cache for that
// file so the next message handler call picks up the new code.
// Bot stays connected — no restart needed.
const watchTargets = [
    require.resolve(__filename),
    require.resolve('../faro.js'),
    require.resolve('./FaroFunctions.js'),
    require.resolve('./FaroPoll.js'),
    require.resolve('./FaroCMD.js'),
    require.resolve('./setting.js'),
    require.resolve('./FaroDownloader.js'),
]
    ...
    fs.watchFile(filePath, { interval: 500 }, () => {
        ...
        debounce = setTimeout(() => {
            try {
                ...
                console.log(`\x1b[33m[Hot Reload] Updated: ${path.basename(filePath)}\x1b[0m`)
            } catch (e) {
                console.error(`[Hot Reload] Error clearing cache for ${path.basename(filePath)}:`, e.message)
            }
        }, 300)
    })
}

watchTargets.forEach(hotReload)