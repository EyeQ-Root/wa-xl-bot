...
module.exports = async (command, faro, rl, callback) => {
    try {
        ...
        function mainMenu() {
            ...
            rl.once("line", async (input) => {
                ...
                switch (input) {
                    ...
                    case "6": {
                        await showSubMenu("Sticker Settings", [
                            `Author: ${settings.stickerAuthor || "Faro"}  - Enter new author`,
                            `Pack:   ${settings.stickerPack || "xl"}   - Enter new pack name`,
                            `Dev Numbers (current: ${(settings.devNumbers || []).join(", ")}) - Enter JID`,
                        ], {
                            1: () => {
                                showSubMenu(
                                    () => `Current author: ${settings.stickerAuthor || "Faro"} - Enter new`,
                                    [],
                                    { default: (x) => { updateSettings("stickerAuthor", x.trim()) } },
                                    true
                                )
                            },
                            2: () => {
                                showSubMenu(
                                    () => `Current pack: ${settings.stickerPack || "xl"} - Enter new`,
                                    [],
                                    { default: (x) => { updateSettings("stickerPack", x.trim()) } },
                                    true
                                )
                            },
                            3: () => {
                                showSubMenu(
                                    () => `201006471515@s.whatsapp.net`,
                                    [],
                                    {
                                        default: (x) => {
                                            const current = settings.devNumbers || []
                                            if (!current.includes(x.trim())) current.push(x.trim())
                                            updateSettings("devNumbers", current)
                                        }
                                    },
                                    true
                                )
